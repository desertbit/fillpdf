...
// Overlay a stamp PDF on another PDF file.
// One variadic boolean specifies, whenever to overwrite the destination file if it exists.
func Stamp(stampPDFFile, srcPDFFile, destPDFFile string, overwrite ...bool) (err error) {
	// Get the absolute paths.
	stampPDFFile, err = filepath.Abs(stampPDFFile)
	if err != nil {
		return fmt.Errorf("failed to create the absolute path: %v", err)
	}
	srcPDFFile, err = filepath.Abs(srcPDFFile)
	if err != nil {
		return fmt.Errorf("failed to create the absolute path: %v", err)
	}
	destPDFFile, err = filepath.Abs(destPDFFile)
	...
	e, err := exists(stampPDFFile)
	if err != nil {
		return fmt.Errorf("failed to check if stamp PDF file exists: %v", err)
	} else if !e {
		return fmt.Errorf("stamp PDF file does not exists: '%s'", stampPDFFile)
	}

	// Check if the src file exists.
	e, err = exists(srcPDFFile)
	if err != nil {
		return fmt.Errorf("failed to check if src PDF file exists: %v", err)
	} else if !e {
		return fmt.Errorf("src PDF file does not exists: '%s'", srcPDFFile)
	}
	...
	// Create a temporary directory.
	tmpDir, err := ioutil.TempDir("", "stamppdf-")
	if err != nil {
		return fmt.Errorf("failed to create temporary directory: %v", err)
	}
